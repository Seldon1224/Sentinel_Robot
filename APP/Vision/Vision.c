...
{
	//判断帧头数据是否为0xA5
	if (ReadFormUsart[0] == VISION_SOF && ReadFormUsart[VISION_RX_LENGTH-1] == VISION_TOF)
	{
		//接收数据拷贝
		memcpy(&VisionRecvData, ReadFormUsart, VISION_RX_LENGTH);
//		time (&timep);
//		delta_time = timep - VisionRecvData.time;
//		VisionRecvData.time = (uint64_t)VisionRecvData.time;
//		VisionRecvData.time -= VISION_TIME_DELTA;
